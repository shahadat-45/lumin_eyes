...
//        dd($frontcategories);
        ...
        $sliders = Banner::where(['status' => 1, 'category_id' => 1])
            ...
            ->get();
        $adBanners= Banner::where(['status' => 1, 'category_id' => 8])
            ->select('id', 'image', 'link')
            ->first();  
        ...
        $flashProducts= Product::where(['status' => 1, 'flashSale' => 1])
            ->orderBy('id', 'DESC')
            ->select('id', 'name', 'slug', 'new_price', 'old_price')
            ->with('prosizes', 'procolors','image','images')
            ->limit(6)
        ...
        $forYouProducts = Product::where('status', 1)
            ->with('prosizes', 'procolors', 'image', 'images')
            ->inRandomOrder()
            ->limit(18)
            ->get();
       
        return view('frontEnd.layouts.pages.index', compact('sliders', 'frontcategories', 'hotdeal_top', 'hotdeal_bottom', 'homeproducts', 'sliderbottomads', 'footertopads','newCategories','featuredCategories','offerProducts','adBanners','flashProducts','forYouProducts'));
    }
    ...
    public function allProducts()
    {
        ...
        return view('frontEnd.layouts.pages.all-products', compact('products'));
    }
    ...
    public function contactSave(Request $request)
    {
        ...
        $rules = [
            'name' => 'required|string|max:255',  // Ensure 'name' is a required string, max length 255
            'phone' => 'required|string|max:15', // Ensure 'phone' is required, max length 15
            'email' => 'required|email|max:255', // Ensure 'email' is a valid email and max length 255
            'message' => 'required|string|max:500', // Ensure 'message' is a required string, max length 500
        ];
        ...
        // If validation fails, redirect to the `#contact` section with errors and old input
        if ($validator->fails()) {
            return redirect()->to(url('blogs/#contact'))
                ->withErrors($validator) // Attach validation errors
                ->withInput(); // Retain old input
        }
    
        // Save the valid data to the database
        $contact = new ContactList();
        $contact->name = $request->input('name');  // Use `input()` to fetch the sanitized value
        $contact->phone = $request->input('phone');
        $contact->email = $request->input('email');
        $contact->message = $request->input('message');
        $contact->save();
    
        // Redirect to `#contact` with a success message
        return redirect()->to(url('blogs/#contact'))
